...
export default function Jobs() {
  ...
  async function loadJobs() {
    ...
    try {
      ...
      if (error) {
        ...
        setLastError(error.message || "Failed to load jobs.");
        setJobs([]);
      } else {
      ...
      console.error("[JOBS] SELECT unexpected error:", err);
  ...
  const filtered = useMemo(() => {
    ...
    if (!t) return jobs;
    return jobs.filter(
      (j) => j.job_code.toLowerCase().includes(t) || j.address.toLowerCase().includes(t)
    );
  }, [jobs, q]);
    ...
    <div className="bg-white rounded-2xl shadow-card p-6">
      <div className="flex items-center justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-subink">Browse all jobs. Click a job to view its report folders.</p>
          <div className="mt-2 text-xs text-subink">
            <strong>Debug:</strong> user={user?.email ?? "null"} | role {role ?? "null"}
          </div>
        </div>
...
function AddJobModal({ onClose, onSaved, setLastError }) {
  ...
  async function save() {
    ...
    try {
      const resp = await supabase.from("jobs").insert(payload, { returning: "minimal" });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-card p-6">
        ...
        <label className="block mb-2 text-sm font-medium text-subink">Job Code (e.g. BBN.4310)</label>
        <input
          value={jobCode}
        ...
        <label className="block mb-2 text-sm font-medium text-subink">Site Address</label>
        <input
          value={address}
        ...
        <div className="flex items-center justify-end gap-3">
          <button onClick={onClose} className="rounded-lg border border-gray-300 px-4 py-2 hover:bg-gray-50" disabled={saving}>
            Cancel
          </button>
          <button onClick={save} disabled={saving} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition disabled:opacity-60">
